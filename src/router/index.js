 ...
 *   roles: ['admin', 'leader', 'member', 'Unauthorized']   // 可访问角色列表
 * }
 *
 */
import {createRouter, createWebHistory} from 'vue-router'
import {USER_PERMISSIONS_KEY} from '@/config/settings.js'
...
function isUnauthorizedOnly(meta) {
  return Array.isArray(meta?.roles) && meta.roles.length === 1 && meta.roles[0] === 'Unauthorized'
}

// 访问控制：已登录不再继承公共未登录页
function hasAccess(metaRoles, role) {
  if (!Array.isArray(metaRoles)) return false
  if (role === 'Unauthorized') return metaRoles.includes('Unauthorized')
  // 已登录用户只访问包含自身角色的路由
  return metaRoles.includes(role)
}

// 公共初始路由（未登录可访问）
const publicRoutes = allRoutes.filter(r => hasAccess(r.meta?.roles, 'Unauthorized'))
...
router.beforeEach((to, from, next) => {
  ...
  if (localStorage.getItem(USER_PERMISSIONS_KEY) !== 'Unauthorized' && isUnauthorizedOnly(to.meta)) {
    return next({path: '/'})
  }
